    ...
    <div class="container">
        ...
        <form action="{{ route('admin.aktivitas.update', $aktivitas->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="nama_aktivitas" name="nama_aktivitas" value="{{ $aktivitas->nama_aktivitas }}" >
            </div>
            <div class="mb-3">
                <label for="tipe" class="form-label">Tipe Aktivitas</label>
                <select id="tipe" name="tipe" class="form-select" >
                    <option value="Reguler" {{ old('tipe', $aktivitas->tipe) == 'Reguler' ? 'selected' : '' }}>Reguler</option>
                    <option value="Half Day" {{ old('tipe', $aktivitas->tipe) == 'Half Day' ? 'selected' : '' }}>Half Day</option>
                    <option value="Full Day" {{ old('tipe', $aktivitas->tipe) == 'Full Day' ? 'selected' : '' }}>Full Day</option>
                </select>
            </div>
            <div class="mb-3">
                <label for="urutan" class="form-label">Urutan</label>
                <input type="number" name="urutan" class="form-control" value="{{ $aktivitas->urutan }}" >
            </div>
